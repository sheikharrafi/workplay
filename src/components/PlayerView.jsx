...
export default function PlayerView({ video, relatedVideos = [], onVideoSelect, onBack, onToggleFavorite, onUpdateVideo, onIncrementViewsAndPlays, currentUser, onDeleteVideo, onShareVideo, settings = { autoplay: true, rememberProgress: true, resolution: 'auto' } }) {
  ...
  const handleDoubleClick = event => { const rect = containerRef.current?.getBoundingClientRect(); if (rect) skip(event.clientX - rect.left < rect.width / 2 ? -10 : 10); };
  const progressPercent = duration ? (currentTime / duration) * 100 : 0;

  useEffect(() => {
    const onKeyDown = event => { if (['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName)) return; if (event.key === ' ') { event.preventDefault(); handlePlayPause(); } else if (event.key === 'ArrowLeft') { event.preventDefault(); skip(-10); } else if (event.key === 'ArrowRight') { event.preventDefault(); skip(10); } else if (event.key.toLowerCase() === 'f') { event.preventDefault(); toggleFullscreen(); } else if (event.key.toLowerCase() === 'm') { event.preventDefault(); toggleMute(); } else if (event.key === '?') setShowShortcuts(value => !value); };
