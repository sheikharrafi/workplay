...
(() => {
  ...
  if (savedTheme === 'light') {
    ...
    root.classList.toggle('dark', isDark); root.classList.toggle('light', !isDark);
...
function AppShell() {
  ...
  useEffect(() => {
    if (currentUser && location.pathname === '/auth') navigate('/', { replace: true });
  ...
  const handleVideoSelect = (video) => {
    ...
    const newRecord = {
      ...
      resolution: video.resolution || 'Auto',
  ...
  const handleDeleteVideo = (videoId) => {
    ...
    setConfirmDialog({ isOpen: true, title: 'Delete Video', message: `Are you sure you want to delete "${title}" from your library?`, onConfirm: () => {
      ...
      if (currentUser) {
        set(ref(db, `users/${currentUser.uid}/videos`), updatedVideos.length ? updatedVideos : null).catch(() => {});
        set(ref(db, `users/${currentUser.uid}/history`), updatedHistory.length ? updatedHistory : null).catch(() => {});
        set(ref(db, `users/${currentUser.uid}/progress/${videoId}`), null).catch(() => {});
      } else {
        setVideosInDb(updatedVideos); setHistoryInDb(updatedHistory);
      }
  ...
  // History stores a complete playable snapshot, so it still opens even when the library state has not loaded yet.
  const handlePlayFromHistory = (videoId, historyItem) => {
    const matched = videosRef.current.find(v => String(v.id) === String(videoId));
    const playableVideo = matched || (historyItem?.videoUrl ? {
      id: historyItem.videoId,
      title: historyItem.title,
      size: historyItem.size,
      duration: historyItem.duration,
      thumbnail: historyItem.thumbnailUrl || historyItem.thumbnail,
      thumbnailUrl: historyItem.thumbnailUrl || historyItem.thumbnail,
      videoUrl: historyItem.videoUrl,
      downloadUrl: historyItem.downloadUrl,
      originalUrl: historyItem.originalUrl,
      resolution: historyItem.resolution || 'Auto',
  ...
  const onResetData = () => handleResetData(() => {
    if (currentUser) {
      ...
      localStorage.removeItem('teraplay_videos'); localStorage.removeItem('teraplay_history'); setVideosInDb([]); setHistoryInDb([]);
    }
  });
      ...
      <main className={`flex-1 ${sidebarCollapsed ? 'md:ml-20' : 'md:ml-64'} transition-all duration-300 p-4 pt-24 md:p-10 w-full min-h-screen pb-28 md:pb-10 box-border flex flex-col`}>
        <Suspense fallback={<SuspenseFallback />}>
          <Routes>
            ...
            <Route path="/discover" element={<DiscoverView videos={discoverVideos} onVideoSelect={handleVideoSelect} />} />
